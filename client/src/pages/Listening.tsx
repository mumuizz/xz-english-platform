...
export default function Listening() {
  ...
  const filteredMaterials = useMemo(() => materials.filter((item) => (scene === 'all' || item.tags.includes(scene)) && (level === 'all' || item.level === level)), [materials, scene, level])
  ...
  const retryQuestions = useMemo(() => wrongNotebook.slice(0, 5), [wrongNotebook])
            ...
            <section className="rounded-3xl bg-white p-8 shadow-md">
              <div className="mb-6 flex items-center justify-between">
                ...
                <div className="flex gap-3">
                  ...
                  <div className="rounded-full bg-[#ef233c] px-4 py-2 text-sm font-semibold text-white">{wrongNotebook.length} 题</div>
                </div>
              </div>
                    ...
                    {retryQuestions.map((item, index) => (
                      <div key={`${item.materialTitle}-${item.questionId}`} className="rounded-2xl bg-white p-5">
                        ...
                        <div className="mt-4 space-y-2">
                          {[item.correctAnswer, item.selectedAnswer, '跳过本题', '重新判断'].filter((v, i, arr) => arr.indexOf(v) === i).map((option) => {
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-start justify-between gap-3">
                        <div><div className="text-lg font-bold text-[#2b2d42]">{material.title}</div><div className="mt-1 text-sm text-[#8d99ae]">{material.titleZh}</div></div>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
              <div className="flex max-h-[92vh] w-full max-w-6xl flex-col rounded-3xl bg-white shadow-2xl">
                <div className="flex items-start justify-between border-b border-[#e9ecef] p-6">
                  <div className="max-w-3xl"><h2 className="text-3xl font-bold text-[#2b2d42]">{selectedMaterial.title}</h2><p className="mt-2 text-[#8d99ae]">{selectedMaterial.titleZh}</p></div>
                  <button onClick={() => { setSelectedMaterial(null); setIsPlaying(false); audioRef.current?.pause() }} className="flex h-10 w-10 items-center justify-center rounded-full bg-[#f8f9fa] text-[#2b2d42]">X</button>
                </div>
                <div className="grid flex-1 gap-6 overflow-y-auto p-6 lg:grid-cols-[1fr,1fr,1fr] lg:p-8">
                  <div className="space-y-6">
                    <div className="rounded-3xl bg-gradient-to-br from-[#ef233c] to-[#c41c30] p-8 text-white shadow-xl">
                      ...
                      <div className="mt-6"><input type="range" min="0" max={selectedMaterial.duration} value={currentTime} onChange={(event) => { const time = Number.parseFloat(event.target.value); setCurrentTime(time); if (audioRef.current) audioRef.current.currentTime = time }} className="w-full accent-white" /><div className="mt-2 flex justify-between text-sm text-white/80"><span>{formatTime(currentTime)}</span><span>{formatTime(selectedMaterial.duration)}</span></div></div>
                      <div className="mt-6 flex flex-wrap items-center gap-3">
                        <button onClick={togglePlay} className="rounded-2xl bg-white px-6 py-3 font-semibold text-[#ef233c]">{isPlaying ? '暂停' : '播放'}</button>
                        {[0.75, 1, 1.25, 1.5].map((rate) => <button key={rate} onClick={() => { setPlaybackRate(rate); if (audioRef.current) audioRef.current.playbackRate = rate }} className={`rounded-xl px-4 py-2 text-sm font-semibold ${playbackRate === rate ? 'bg-white text-[#ef233c]' : 'bg-white/15 text-white'}`}>{rate}x</button>)}
                      </div>
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      <div className="mb-4 flex items-center justify-between"><h3 className="text-lg font-bold text-[#2b2d42]">听写模式</h3>{dictationSubmitted && <span className="rounded-full bg-[#2b2d42] px-3 py-1 text-sm font-semibold text-white">得分 {dictationScore}/{dictationSegments.length}</span>}</div>
                      <div className="space-y-4">{dictationSegments.map((segment, index) => { const correct = normalize(dictationAnswers[segment.id] || '') === normalize(segment.answer); return <div key={segment.id} className="rounded-2xl border border-[#e9ecef] bg-white p-5"><div className="font-semibold text-[#2b2d42]">{index + 1}. {segment.prompt}</div><input value={dictationAnswers[segment.id] || ''} onChange={(event) => setDictationAnswers((prev) => ({ ...prev, [segment.id]: event.target.value }))} disabled={dictationSubmitted} placeholder="输入听写结果" className="mt-4 w-full rounded-xl border border-[#d1d5db] px-4 py-3 outline-none" />{dictationSubmitted && <div className={`mt-4 rounded-xl p-4 text-sm ${correct ? 'bg-[#10b981]/10 text-[#065f46]' : 'bg-[#ef233c]/10 text-[#991b1b]'}`}>{correct ? '听写正确' : `正确答案：${segment.answer}`}</div>}</div> })}</div>
                      {dictationSegments.length > 0 && <div className="mt-4 flex gap-3"><button onClick={() => setDictationSubmitted(true)} disabled={dictationSubmitted || Object.keys(dictationAnswers).length < dictationSegments.length} className={`flex-1 rounded-2xl px-5 py-3 font-semibold text-white ${dictationSubmitted || Object.keys(dictationAnswers).length < dictationSegments.length ? 'cursor-not-allowed bg-gray-400' : 'bg-gradient-to-r from-[#2b2d42] to-[#3f4564]'}`}>提交听写</button><button onClick={() => { setDictationAnswers({}); setDictationSubmitted(false) }} className="rounded-2xl border border-[#d1d5db] px-5 py-3 font-semibold text-[#2b2d42]">重做</button></div>}
                    </div>
                  </div>
